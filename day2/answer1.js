...
const regexpGame = /Game (?<gid>[0-9]+):\s+(?<seqs>.*)/;
const redSeq = /((?<red>[0-9]+) red)/;
const greenSeq = /((?<green>[0-9]+) green)/;
const blueSeq = /((?<blue>[0-9]+) blue)/;

let sum = 0;

input.split("\n").forEach((line) => {
  const gameMatch = line.match(regexpGame);
  const gameId = parseInt(gameMatch.groups.gid, 10);

  let valid = true;

  for (let seq of gameMatch.groups.seqs.split(";")) {
    const redMatch = seq.trim().match(redSeq);
    const greenMatch = seq.trim().match(greenSeq);
    const blueMatch = seq.trim().match(blueSeq);

    const r = parseInt((redMatch && redMatch.groups.red) || 0);
    const g = parseInt((greenMatch && greenMatch.groups.green) || 0);
    const b = parseInt((blueMatch && blueMatch.groups.blue) || 0);

    if (r > gameConfig.red || g > gameConfig.green || b > gameConfig.blue) {
      valid = false;
      break;
    }
  }
